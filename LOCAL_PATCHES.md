...
## 7. `src/store.js` — `listCards` 加 `limit: 0` 取全量口（修上游 100 张硬顶，2026-07-19）
**上游 bug**：`listCardInbox`(:1076) 与 `listCardCollection`(:1090) 都传 `limit: 10_000` 意在「取全量再自己过滤/分页」，但 `listCards`(:1054) 里 `Math.min(Math.max(Number(limit)||20,1),100)` 把 10000 **钳到 100** —— 全库只有最新 100 张卡进得了过滤管线。
**后果**：卡片攒过 100 张后，收藏页 `total` 在 100 封顶、`offset>100` 恒返回空、**第 101 张之后的老卡永久不可达**。手机批要把收藏做成主要入口，这坑必爆。
**补丁**：`limit === 0` 视作「不限量」的**内部**取全量口（`Number.MAX_SAFE_INTEGER`），两个调用点改传 `limit: 0`。
